...
import com.example.madcamp_week2.ui.chat.Chat;
import com.example.madcamp_week2.ui.chat.Code;

import java.util.ArrayList;

public class ChatRcvAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private ArrayList<Chat> chatList;

    public ChatRcvAdapter(ArrayList<Chat> chatList){
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        if(viewHolder instanceof CenterViewHolder){
            ((CenterViewHolder)viewHolder).textv.setText(chatList.get(position).getContent());
        }else if(viewHolder instanceof LeftViewHolder){
            ((LeftViewHolder)viewHolder).textv_nicname.setText(chatList.get(position).getName());
            ((LeftViewHolder)viewHolder).textv_msg.setText(chatList.get(position).getContent());
        }else{
            ((RightViewHolder)viewHolder).textv_msg.setText(chatList.get(position).getContent());
        }
